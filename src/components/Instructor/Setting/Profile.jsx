...
const ProfileIN = () => {
  ...
  useEffect(() => {
    const fetchProfilePicture = async () => {
      try {
        ...
        const response = await fetch(
          "https://course-compass-backend-zh7c.onrender.com/api/instructor/profile/picture",
          {
            method: "GET",
            headers: {
    ...
    const fetchAbout = async () => {
      try {
        const token = await auth.currentUser.getIdToken();
        const response = await fetch(
          "https://course-compass-backend-zh7c.onrender.com/api/instructor/about",
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const updateAbout = async () => {
    try {
      const token = await auth.currentUser.getIdToken();
      const response = await fetch(
        "https://course-compass-backend-zh7c.onrender.com/api/instructor/update/about",
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div className="px-4 py-4">
        ...
        <div>
          ...
          <div className="flex space-x-5 justify-end">
            <button
              className="px-4 py-1 rounded-lg bg-black font-inter text-white"
              onClick={updateAbout}
            >
              Update
            </button>
